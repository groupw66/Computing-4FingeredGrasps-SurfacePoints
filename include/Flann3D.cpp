...
int Flann3D::knnSearch(const std::vector<Eigen::Vector3d>& points, std::vector< std::vector< std::tuple<Eigen::Vector3d, int, double> > >& out, int knn)
{
    ...
    for(int i=0 ; i < nQuery ; ++i){
        ...
        for(int j=0 ; j < knn ;++j){
            ...
            out[i][j] = std::make_tuple(Eigen::Vector3d(position[0], position[1], position[2]), indices[i][j], sqrt(dists[i][j]));
        }
    }
    return 0;
}

int Flann3D::knnSearch(const Eigen::Vector3d& point, std::vector< std::tuple<Eigen::Vector3d, int, double> >& out, int knn)
{
    ...
    for(int j=0 ; j < knn ;++j){
        ...
        out[j] = std::make_tuple(Eigen::Vector3d(position[0], position[1], position[2]), indices[0][j], sqrt(dists[0][j]));
    }
    return 0;
